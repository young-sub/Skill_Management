...
대상 프로젝트 루트에서 실행한 뒤 설치할 Skill과 적용 범위를 선택합니다. Harness v3 전체 흐름에는 `setup-agent-harness`, `design-goal`, `execute-codex-goal`, `diagnose`, `close-goal`, `maintain-agent-harness`를 선택하고, 아이디어 탐색이 필요하면 `explore-idea`를 추가합니다. 적용 범위는 사용할 에이전트와 Project를 선택합니다. `-g`를 사용하지 않으므로 프로젝트 로컬에 설치됩니다.
...
```text
$setup-agent-harness로 이 새 저장소를 Harness v3로 설정해줘.
```
...
```text
$setup-agent-harness로 기존 구조를 먼저 조사하고 이 저장소를 Harness v3로 설정해줘.
```

### 기존 프로젝트 — Harness v2 적용

위 명령으로 같은 이름의 Harness Skill을 v3로 다시 설치한 뒤 마이그레이션을 요청합니다. 선택하지 않은 기존 Skill은 자동 삭제되지 않으며, 폐기된 `project-agent-bootstrap` 별칭이나 기존 구조의 삭제·이동은 조사 결과와 승인 후 정리합니다.

```text
$setup-agent-harness로 기존 v2 설정을 조사하고 Harness v3로 마이그레이션해줘. 삭제나 이동은 적용 전에 계획을 보여줘.
```
