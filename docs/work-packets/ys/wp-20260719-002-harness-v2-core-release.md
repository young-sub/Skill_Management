...
- external verification WP-06: `powershell -NoProfile -File scripts/test-install.ps1 -VerifyUpdate` remains `not_run`; the sandbox escalation request for external `npx` execution was rejected, so explicit approval is still required
- risks: WP-06 real install/update smoke remains unverified pending external execution approval; live GitHub release is not authorized
- next mode: run
- next stop condition: explicit approval for the external `npx` install/update smoke
