...
- Post-merge hardening: a fresh Windows checkout exposed CRLF-sensitive generated-resource and manifest hashes; RED regression tests now cover checkout conversion and the maintenance consumer, and both producer and consumer hash normalized text while retaining raw hashing for binary resources. Clean CRLF text is accepted while substantive text and binary edits remain detectable.
