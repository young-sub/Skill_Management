...
Use the Goal contract lifecycle only when the global Work Packet threshold is met or the user explicitly selects it. Within that workflow, capability work uses one to five canonical Items; use one when the change has only one meaningful review boundary. A Korean Design Review shows What, How, Test, and Done with stable IDs and behavior visuals. A valid ordinary design is authorized by default against the canonical Review; the user can veto it without performing approval ceremony. Host Goal tracking is optional. Implement dependency-ready core Items first, run mapped Impacted checks, and commit independently reviewable slices when project policy or the user requires intermediate commits.
...
Close evaluates Items independently, updates only mapped durable truth, renders the aligned Korean Result Review, and moves work to typed retained state. Re-resolve captured base movement and protection before integration. `main` is protected in this repository; leave a verified branch for human integration and never auto-push.
