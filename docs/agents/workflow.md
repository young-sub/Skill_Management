...
Close evaluates Items independently, updates only mapped durable truth, writes `result.json`, summarizes the outcome in final chat, and moves work to typed retained state. Re-resolve captured base movement and protection before integration. `main` is protected in this repository; leave a verified branch for human integration and never auto-push.
