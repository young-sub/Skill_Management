...
Full runs only for a configured executable trigger: shared runtime, schema, test discovery, repository workflow cutover, or an explicit request. Generated prose, Skill instructions, Review templates, and manifests use focused renderer/resource/distribution checks rather than triggering Full by themselves. `not_required` names the matched impact rule; it never means passed.
