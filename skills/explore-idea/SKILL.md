...
If the user later authorizes implementation, switch to the appropriate workflow and proceed. Small, clear changes use direct development; multiple review boundaries, material risk, unresolved product decisions, or an explicit contract request can use `design-goal`. Do not require the user to invoke another skill or repeat approval. The exploration request alone does not authorize implementation.
