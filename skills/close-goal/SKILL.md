...
<!-- Source-SHA256: 0742382003cefdab04baa28f691c6956dccd49b40377bd901a2f092e5700ab0a -->
...
1. Match Result Items to contract IDs/order, planned checks when present, and Done criteria. Reconcile observable implementation, actual check results, and planned-versus-actual deltas. An empty test list is valid when Done evidence proves a docs/tooling-only or behavior-neutral Item. Result validation checks consistency; it does not run commands or authenticate submitted evidence. Inspect the underlying evidence before claiming completion.
...
3. Failed or required-but-unrun checks leave affected Items incomplete; no behavior check is required for `scope=none` when its Done evidence is sufficient. Unknown impact needs investigation. Reuse existing authorization for material deltas within scope; ask only about an uncovered decision or risk. Do not erase failures or add synthetic checks to close the contract.
