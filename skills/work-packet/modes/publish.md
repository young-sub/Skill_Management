...
- Always read: this file, the `Phase handoff capsule` if present, the pending local Issue/PR body files, `git status --short`, `git diff --stat` or changed filenames, and these sections via `scripts/read-reference-section.py`: `Access path resolution`, `Metadata-first Tracker I/O`, `Durable Body Ownership`, `Tracker and durable record policy`, `Document layout and merge safety`, `Branch naming policy`, `Final active-branch refresh policy`, and `Verification evidence`.
