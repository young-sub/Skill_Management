...
7. If the input is a bug, failing verification, flaky behavior, or performance problem, choose `diagnose_first` and delegate to `diagnosing-bugs` to establish the feedback loop before planning the fix.
...
9. If the full grill threshold is met, choose `full_grill_with_docs`, delegate to `grill-with-docs`, and use `domain-modeling` when vocabulary or an ADR boundary must be made explicit. Present the Decision Map from `REFERENCE.md`, ask one fixed-format question at a time, and stop. Strengthen full grill with the Full Grill Decision Tree Protocol, but keep the user-facing output to the Decision Map/status plus the next fixed-format question. Do not synthesize a roadmap, PRD summary, vertical slices, acceptance criteria, Implementation Contract, Codex goal, or Implementation Confirmation Brief while direct grill questions remain unanswered. If an isolated local file is useful, fill only a decision scaffold using `templates/grill-decision-map.md`, not a proposed implementation plan.
...
12. For `prototype_first`, delegate to `prototype`; capture what the prototype answers and where the durable answer must be recorded. For evidence-heavy uncertainty that does not require a prototype, delegate bounded primary-source work to `research` and retain its artifact reference.
13. For `architecture_first`, perform bounded architecture discovery or delegate to `improve-codebase-architecture`, using `codebase-design` as the boundary/interface vocabulary; do not turn it into a broad refactor.
...
15. If enough context exists, delegate to `to-spec` to synthesize the settled specification after alignment. In `local_markdown` mode it writes only to the configured gitignored `docs/plans/<owner>/<feature>/spec.md` surface and performs no remote tracker operation.
