...
3. Delegate a final standards/spec pass to `code-review` when available. Apply `improve-codebase-architecture` as a scoped review of touched modules, interfaces, seams, adapters, tests, and diagnostics, not as a broad refactor. Record outcome, changed files or commits, verification evidence, docs updated, review results, remaining risks, and unverified assumptions. Follow the `Korean reporting and summary policy` reference section and `templates/korean-summary.md`; for close reports, emphasize completed outcome, tracker/PR state, remaining decision, and next action only when they affect follow-up.
...
9. Do not leave shared durable knowledge only in `.scratch/` or a gitignored personal plan. For a configured ignored `docs/plans/<owner>/<feature>/` surface, keep the personal files local and mirror settled source-of-truth decisions plus verification evidence into tracked docs/ADRs or the PR body when one exists. Only remote-publish records move to their configured tracked archive after publish. Update any shared index/pointer in the serialized orchestration lane per the `Document layout and merge safety` reference section.
