...
9. Do not leave durable review knowledge only in `.scratch/`. When the Work Packet's design or planning docs (PRD, implementation plan, design notes, decision records) are completed or superseded by the implemented result, move them to `docs/archive/...` with decision history and verification evidence, keeping active plans out of the archive. Update any index/pointer in the serialized orchestration lane per the `Document layout and merge safety` reference section (regenerate a derived index, or append one stably-ordered line; do not reflow the shared file).
