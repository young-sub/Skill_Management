...
- Small, reversible edits: inspect, change, verify, and report. No separate plan file, Work Packet, or Harness setup is required.
...
- Use a Work Packet for multiple reviewable boundaries, material risk, unresolved product decisions, or an explicit request. Aim for one reviewable PR; split only independently working units.
- Invoke setup or Harness repair only when requested. Missing optional Harness configuration must not turn an ordinary task into a setup project.
...
- Ordinary work history belongs in Git or the repository's ephemeral work area. Retain completed plans only for an explicit release, legal, or audit requirement.
...
- Subagents inherit the current model unless the user chooses another. This user's minimum is GPT-5.6 Sol; GPT-6 Astra is also allowed. Never select a lower model to save cost. Adjust reasoning to task difficulty.
...
- Before every global AGENTS.md revision, preserve each target's exact previous bytes under `C:/Users/YS/Desktop/python/Skill_Management/back-up/global-agents/<date-time>/`, with its source path and SHA256. Verify the backup before replacement and never overwrite historical snapshots.
- The maintained global source and backup procedure live in `C:/Users/YS/Desktop/python/Skill_Management/authoring/global/AGENTS.md` and `docs/agents/global-guidance.md` in that repository. Backup history is retained by explicit user request.
